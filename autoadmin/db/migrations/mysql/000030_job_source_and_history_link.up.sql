...
-- Filebeat 作业名首字母大写（"Filebeat install"），故用 LOWER 比较（PG 侧大小写敏感）。
UPDATE monitor_target_install_history h
JOIN automation_execution_job j
  ON j.create_time = h.create_time
 AND j.template_name_snapshot = h.exporter_type_snapshot
 AND LOWER(j.task_name_snapshot) = LOWER(CONCAT(h.exporter_type_snapshot, ' ', h.action))
SET h.automation_job_id_snapshot = j.id
WHERE h.automation_job_id_snapshot IS NULL;

-- 按历史关联把监控安装作业的来源回填为 monitor_target。
UPDATE automation_execution_job j
JOIN monitor_target_install_history h ON h.automation_job_id_snapshot = j.id
SET j.source = 'monitor_target'
WHERE j.source = 'manual';

-- Agent 安装/更新作业按主机日志里的 agent_job_id 回填来源。
UPDATE automation_execution_job j
SET j.source = 'agent_install'
WHERE j.source = 'manual'
  AND EXISTS (
