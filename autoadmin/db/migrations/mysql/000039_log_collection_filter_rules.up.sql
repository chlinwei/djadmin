...
ALTER TABLE `monitor_log_collection_filter_rule`
  ADD COLUMN `rule_type` varchar(16) NOT NULL DEFAULT 'include' AFTER `pattern`;

ALTER TABLE `assets_application_log_definition`
  ADD COLUMN `filter_include_rule_id` bigint DEFAULT NULL AFTER `processing_rule_id`,
  ADD COLUMN `filter_exclude_rule_id` bigint DEFAULT NULL AFTER `filter_include_rule_id`;

ALTER TABLE `assets_application_service_log_setting`
  ADD COLUMN `collection_exclude_filter_rule_id` bigint DEFAULT NULL AFTER `collection_filter_rule_id`;
