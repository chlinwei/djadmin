...
ALTER TABLE `automation_playbook_template`
  ADD COLUMN `content_format` varchar(16) NOT NULL DEFAULT 'playbook' AFTER `content`;

-- 作业快照同步记录模板形态：改模板形态不影响在途作业（与 template_content_snapshot 同语义）。
ALTER TABLE `automation_execution_job`
  ADD COLUMN `template_content_format_snapshot` varchar(16) NOT NULL DEFAULT 'playbook' AFTER `template_content_snapshot`;
