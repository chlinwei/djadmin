...
UPDATE sys_menu SET name = '任务执行', update_time = CURDATE()
...
SELECT '任务新增', NULL, parent_id, 8, '', NULL, 'F', 'automation:tasks:create', CURDATE(), CURDATE(), '自动化任务新增', location, 1
FROM sys_menu WHERE perms = 'automation:tasks:view'
  AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'automation:tasks:create');

INSERT INTO sys_menu (name, icon, parent_id, order_num, path, component, menu_type, perms, create_time, update_time, remark, location, is_expanded)
SELECT '任务编辑', NULL, parent_id, 9, '', NULL, 'F', 'automation:tasks:update', CURDATE(), CURDATE(), '自动化任务编辑', location, 1
FROM sys_menu WHERE perms = 'automation:tasks:view'
  AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'automation:tasks:update');

INSERT INTO sys_menu (name, icon, parent_id, order_num, path, component, menu_type, perms, create_time, update_time, remark, location, is_expanded)
SELECT '任务删除', NULL, parent_id, 10, '', NULL, 'F', 'automation:tasks:delete', CURDATE(), CURDATE(), '自动化任务删除', location, 1
FROM sys_menu WHERE perms = 'automation:tasks:view'
