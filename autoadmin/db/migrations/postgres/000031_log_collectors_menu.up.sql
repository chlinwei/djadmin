...
--
-- PG 侧差异：CURDATE() → CURRENT_DATE；is_expanded 的 0 → FALSE；INSERT IGNORE → ON CONFLICT DO NOTHING。
...
SELECT '日志采集', 'file-lines', p.id, 4, '/monitor/logging/collectors', 'monitor/log-collectors/index', 'C', 'monitor:log_collect:view', CURRENT_DATE, CURRENT_DATE, 'Filebeat 纳管目标：安装/启停/下发采集配置与配置状态', p.location, FALSE
...
WHERE rm.menu_id = (SELECT id FROM sys_menu WHERE path = '/monitor/logging' AND m.menu_type = 'M' LIMIT 1)
