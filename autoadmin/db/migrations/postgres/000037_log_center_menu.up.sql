...
SELECT '日志中心', 'search', p.id, 5, '/monitor/logging/center', 'monitor/log-center/index', 'C', 'monitor:view', CURRENT_DATE, CURRENT_DATE, '服务树 + 日志配置/查询/本服务水位', p.location, FALSE
...
-- 不补授则菜单树整枝不可见）。ON CONFLICT DO NOTHING 幂等（sys_role_menu 上主键/唯一约束）。
