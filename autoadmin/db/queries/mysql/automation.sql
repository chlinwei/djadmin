...
-- name: CreateAutomationPlaybook :execlastid
...
-- name: CreateAutomationInventory :execlastid
...
-- name: CreateAutomationTask :execlastid
...
WHERE h.id IN (sqlc.slice(host_ids)) AND h.ip IS NOT NULL
ORDER BY h.id;
...
FROM assets_host WHERE id IN (sqlc.slice(host_ids));
