...
-- name: CreateAgentJobHostLog :one
...
WHERE deployment_template_id=sqlc.arg(deployment_template_id) AND id = ANY(sqlc.arg(ids)::bigint[]);
...
DELETE FROM assets_application_service_log_setting WHERE log_definition_id = ANY(sqlc.arg(log_definition_ids)::bigint[]);
...
WHERE deployment_id = ANY(sqlc.arg(deployment_ids)::bigint[]) ORDER BY deployment_id,service_id;

-- ---- 逻辑服务写路径 ----

-- name: CreateApplicationService :one
...
WHERE service_id=sqlc.arg(service_id) AND log_definition_id = ANY(sqlc.arg(log_definition_ids)::bigint[]);
