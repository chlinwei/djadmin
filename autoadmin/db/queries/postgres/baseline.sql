...
-- name: CreateBaseline :one
INSERT INTO baseline(create_time, update_time, name, version, description, enabled)
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id;

-- name: UpdateBaseline :exec
UPDATE baseline SET update_time = $1, name = $2, version = $3, description = $4, enabled = $5 WHERE id = $6;

-- name: DeleteBaseline :execrows
DELETE FROM baseline WHERE id = $1;
...
UPDATE baseline_item SET category_id = $1, name = $2, description = $3, config = $4, severity = $5, update_time = $6
WHERE id = $7;
...
SELECT CAST(COALESCE(MAX(sort), -1) AS bigint) AS max_sort FROM baseline_item WHERE category_id = sqlc.arg(category_id);
...
SELECT CAST(COALESCE(MAX(sort), -1) AS bigint) AS max_sort FROM baseline_category WHERE baseline_id = sqlc.arg(baseline_id);

-- name: CreateBaselineCategory :one
INSERT INTO baseline_category(create_time, update_time, name, sort, baseline_id)
VALUES ($1, $2, $3, $4, $5)
RETURNING id;

-- name: RenameBaselineCategory :execrows
UPDATE baseline_category SET name = $1, update_time = $2 WHERE id = $3;

-- name: UpdateBaselineCategorySort :execrows
UPDATE baseline_category SET sort = $1, update_time = $2 WHERE id = $3;

-- name: DeleteBaselineCategory :execrows
DELETE FROM baseline_category WHERE id = $1;
...
UPDATE security_scan SET status = 'running', start_time = $1, update_time = $2
...
FROM security_scan_target WHERE scan_id = $1 AND host_id = $2;

-- name: FinishBaselineScanTarget :exec
UPDATE security_scan_target
SET status = $1, passed_items = $2, failed_items = $3, compliance_rate = $4, error_message = $5
WHERE id = $6;

-- 仅改状态 + 报错文案（失败/跳过两类终态），不动计数列。
-- name: SetBaselineScanTargetStatus :exec
UPDATE security_scan_target SET status = $1, error_message = $2 WHERE id = $3;
...
UPDATE security_scan SET status = $1, summary = $2, end_time = $3, update_time = $4 WHERE id = $5;
...
UPDATE security_scan SET status = 'canceled', summary = $1, end_time = $2, update_time = $3
WHERE id = $4;
