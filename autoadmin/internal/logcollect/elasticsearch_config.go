...
type elasticsearchClusterInput struct {
...
func (handler *Handler) CreateElasticsearchCluster(context *gin.Context) {
	handler.saveElasticsearchCluster(context, 0)
}

func (handler *Handler) UpdateElasticsearchCluster(context *gin.Context) {
	handler.saveElasticsearchCluster(context, parseID(context.Param("id")))
}

func (handler *Handler) saveElasticsearchCluster(context *gin.Context, id int64) {
	var input elasticsearchClusterInput
	...
	if id == 0 {
		count, err := queries.CountAllElasticsearchClusters(context)
		if err != nil {
			response.Error(context, err)
			return
		}
		if count > 0 {
			response.BusinessError(context, 400, "only one Elasticsearch cluster is supported; edit the existing cluster", nil)
			return
		}
	}
	values, err := handler.elasticsearchClusterValues(input, id == 0)
	if err != nil {
	...
	if isDefault, ok := values["is_default"].(bool); ok && isDefault {
		if err = txQueries.ClearDefaultElasticsearchCluster(context, db.ClearDefaultElasticsearchClusterParams{
	...
	if id == 0 {
		// 新建时 elasticsearchClusterValues(creating=true) 已把未提交的列补上默认值，直接整行插入。
		id, err = txQueries.CreateElasticsearchCluster(context, db.CreateElasticsearchClusterParams{
		...
		merged, mergeErr := mergedElasticsearchCluster(context, queries, id, values)
		...
		affected, updateErr := txQueries.UpdateElasticsearchCluster(context, db.UpdateElasticsearchClusterParams{
	...
	// 集群新增/修改后异步 bootstrap（模板 + ILM 策略），对应 Django sync_log_storage_quietly；
	// 失败只落 storage_sync_* 状态，不阻塞保存。
	go handler.syncClusterLogStorage(id)
	handler.respondElasticsearchCluster(context, id)
}

// mergedElasticsearchCluster 把已提交的字段合并进数据库里的当前行（整行写的输入）。
// 密码列存的是密文，未提交时原样保留（elasticsearchClusterValues 只把提交的新密码加密后放进来）。
...
func mergedElasticsearchCluster(context context.Context, queries *db.Queries, id int64, values map[string]any) (db.MonitorElasticsearchCluster, error) {
	current, err := queries.GetElasticsearchClusterTyped(context, id)
	if err != nil {
		return db.MonitorElasticsearchCluster{}, err
	}
...
func (handler *Handler) elasticsearchClusterValues(input elasticsearchClusterInput, creating bool) (map[string]any, error) {
	...
	if creating {
		...
		for column, value := range map[string]any{"username": "", "password": "", "verify_tls": false, "ca_cert": "", "index_prefix": "autoadmin", "request_timeout": 10, "enabled": true, "is_default": false, "remark": ""} {
...
func (handler *Handler) BatchDeleteElasticsearchClusters(context *gin.Context) {
	batchDeleteMonitorRows(context, handler, handler.deleteElasticsearchClusterByID)
}

// deleteElasticsearchClusterByID 找不到行时返回 sql.ErrNoRows，由批删入口记成 ok:false。
func (handler *Handler) deleteElasticsearchClusterByID(context *gin.Context, id int64) error {
	return deleteRowsAffected(db.New(handler.db).DeleteElasticsearchCluster(context, id))
}
