...
func newPrometheusProxyHandler(t *testing.T, upstream http.HandlerFunc) (*Handler, *httptest.Server) {
	t.Helper()
	...
	return &Handler{db: database, client: upstreamServer.Client()}, upstreamServer
}
...
func TestPrometheusProxyLabelValuesArray(t *testing.T) {
	handler, _ := newPrometheusProxyHandler(t, func(w http.ResponseWriter, r *http.Request) {
