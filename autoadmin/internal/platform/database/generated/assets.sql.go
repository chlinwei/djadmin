...
type CountApplicationsParams struct {
	Pattern   interface{} `json:"pattern"`
	Pattern_2 interface{} `json:"pattern_2"`
	Pattern_3 interface{} `json:"pattern_3"`
	Pattern_4 interface{} `json:"pattern_4"`
}
...
type CountBusinessSystemsParams struct {
	...
	Pattern_3 interface{} `json:"pattern_3"`
	Pattern_4 interface{} `json:"pattern_4"`
	Pattern_5 interface{} `json:"pattern_5"`
}
...
func (q *Queries) CountCredentials(ctx context.Context, arg CountCredentialsParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countCredentials, arg.Pattern, arg.Pattern_2, arg.Pattern_3)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) CountHostGroups(ctx context.Context, arg CountHostGroupsParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countHostGroups, arg.Pattern, arg.Pattern_2)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
WHERE p.name LIKE CAST(? AS CHAR) OR p.code LIKE CAST(? AS CHAR) OR p.owner LIKE CAST(? AS CHAR) OR COALESCE(p.remark, '') LIKE CAST(? AS CHAR)
GROUP BY p.id
