...
SET status = 'cancelled', start_time = COALESCE(start_time, ?), end_time = ?,
    duration_seconds = ?, result_summary = ?,
    update_time = ?
...
UPDATE automation_execution_job
SET status = 'running', start_time = ?, result_summary = ?,
    update_time = ?
...
WHERE h.ip IS NOT NULL
  AND (h.instance_name LIKE ? OR s.hostname LIKE ?
       OR h.ip LIKE ? OR ? IS NULL)
`
...
func (q *Queries) CountAutomationHostOptions(ctx context.Context, arg CountAutomationHostOptionsParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countAutomationHostOptions,
		arg.Pattern,
		arg.Pattern,
		arg.Pattern,
		arg.Pattern,
...
FROM assets_host WHERE id IN (/*SLICE:host_ids*/?)
`
...
func (q *Queries) CountAutomationInventoryHosts(ctx context.Context, hostIds []int64) (CountAutomationInventoryHostsRow, error) {
	query := countAutomationInventoryHosts
...
func (q *Queries) CountAutomationPlaybooks(ctx context.Context, arg CountAutomationPlaybooksParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countAutomationPlaybooks,
		arg.Pattern,
		arg.Pattern,
		arg.Pattern,
		arg.Pattern,
...
WHERE (a.name LIKE ? OR a.remark LIKE ? OR ? IS NULL)
`
...
WHERE (a.id = ? OR ? IS NULL)
  ...
  AND (a.source = ? OR ? IS NULL)
  AND (a.requested_username LIKE ? OR a.template_name_snapshot LIKE ? OR a.task_name_snapshot LIKE ? OR a.remark LIKE ? OR ? IS NULL)
`
...
WHERE (a.id = ? OR ? IS NULL)
  AND (a.name LIKE ? OR p.name LIKE ? OR i.name LIKE ? OR a.remark LIKE ? OR ? IS NULL)
`
...
const createAutomationInventory = `-- name: CreateAutomationInventory :execlastid
...
func (q *Queries) CreateAutomationInventory(ctx context.Context, arg CreateAutomationInventoryParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, createAutomationInventory,
		arg.CreateTime,
		arg.UpdateTime,
		arg.Remark,
		arg.Name,
...
func (q *Queries) CreateAutomationJob(ctx context.Context, arg CreateAutomationJobParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, createAutomationJob,
		arg.CreateTime,
		arg.UpdateTime,
		arg.JobID,
		arg.TaskID,
...
func (q *Queries) CreateAutomationPlaybook(ctx context.Context, arg CreateAutomationPlaybookParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, createAutomationPlaybook,
		arg.CreateTime,
		arg.UpdateTime,
		arg.Remark,
		arg.Name,
...
const createAutomationTask = `-- name: CreateAutomationTask :execlastid
...
func (q *Queries) CreateAutomationTask(ctx context.Context, arg CreateAutomationTaskParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, createAutomationTask,
		arg.CreateTime,
		arg.UpdateTime,
		arg.Remark,
		arg.Name,
...
DELETE FROM automation_inventory WHERE id = ?
`
...
DELETE FROM automation_execution_job_log WHERE job_id = ?
`
...
DELETE FROM automation_playbook_template WHERE id = ?
`
...
DELETE FROM automation_task WHERE id = ?
`
...
SELECT start_time FROM automation_execution_job WHERE id = ?
`
...
func (q *Queries) ListAutomationHostAgentIdentities(ctx context.Context, hostIds []int64) ([]ListAutomationHostAgentIdentitiesRow, error) {
	query := listAutomationHostAgentIdentities
	var queryParams []interface{}
	if len(hostIds) > 0 {
		for _, v := range hostIds {
			queryParams = append(queryParams, v)
...
func (q *Queries) ListAutomationHostOptions(ctx context.Context, arg ListAutomationHostOptionsParams) ([]ListAutomationHostOptionsRow, error) {
	rows, err := q.db.QueryContext(ctx, listAutomationHostOptions,
		arg.Pattern,
		arg.Pattern,
		arg.Pattern,
		arg.Pattern,
...
WHERE h.id IN (/*SLICE:host_ids*/?) AND h.ip IS NOT NULL
ORDER BY h.id
`
...
func (q *Queries) ListAutomationInventoryHosts(ctx context.Context, hostIds []int64) ([]ListAutomationInventoryHostsRow, error) {
	query := listAutomationInventoryHosts
	var queryParams []interface{}
	if len(hostIds) > 0 {
		for _, v := range hostIds {
			queryParams = append(queryParams, v)
...
FROM automation_execution_host_log WHERE job_id = ? ORDER BY id
`
...
SELECT content FROM automation_execution_job_log WHERE job_id = ? ORDER BY id
`
...
ORDER BY
  ...
  CASE WHEN ? = '-id' THEN id END DESC,
  CASE WHEN ? = 'name' THEN name END ASC,
  CASE WHEN ? = '-name' THEN name END DESC,
  CASE WHEN ? = 'create_time' THEN create_time END ASC,
  CASE WHEN ? = '-create_time' THEN create_time END DESC,
  CASE WHEN ? = 'update_time' THEN update_time END ASC,
  CASE WHEN ? = '-update_time' THEN update_time END DESC,
  id DESC
LIMIT ? OFFSET ?
`
...
UPDATE automation_task SET enabled = ?, update_time = ? WHERE id = ?
`
...
SET update_time = ?, remark = ?, name = ?,
    selected_host_ids = ?, enabled = ?,
    update_on_launch = ?, update_cache_timeout = ?
WHERE id = ?
`
...
SET update_time = ?, remark = ?, name = ?,
    playbook_template_id = ?, inventory_id = ?,
    env_vars = ?, default_limit = ?, enabled = ?,
    execution_timeout_seconds = ?, run_as_user = ?,
    run_as_group = ?, work_directory = ?
WHERE id = ?
`
