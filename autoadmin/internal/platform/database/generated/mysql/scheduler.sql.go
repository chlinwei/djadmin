...
WHERE (task_id = ? OR ? IS NULL)
  ...
  AND (duration_seconds >= ? OR ? IS NULL)
  AND (duration_seconds <= ? OR ? IS NULL)
  AND (message LIKE ? OR output LIKE ? OR ? IS NULL)
`
...
FROM scheduler_scheduledtasklog AS l
JOIN scheduler_scheduledtask AS t ON t.id = l.task_id
WHERE (l.task_id = ? OR ? IS NULL)
  AND (l.status = ? OR ? IS NULL)
  AND (l.duration_seconds >= ? OR ? IS NULL)
  AND (l.duration_seconds <= ? OR ? IS NULL)
  AND (l.message LIKE ? OR l.output LIKE ? OR ? IS NULL)
