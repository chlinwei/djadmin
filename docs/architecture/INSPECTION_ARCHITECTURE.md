...
- 保存校验（`validateMountBinding`）：挂载点引用存在性校验（业务/逻辑服务/项目）；
  应用组挂业务/服务必须带实例模式。前端**不暴露挂载编辑控件**——挂载完全由
  "树选中节点"推导（服务→service、业务/环境→business×环境、项目→project），
  弹窗中以 `a-tree-select` 下拉树（复用任务导航树数据：项目→业务→环境→逻辑服务，
  可搜索，输入框回显 `项目->业务->环境->服务` 完整路径）展示和修改"巡检对象"；
  点左侧树节点同样生效。两种入口都会重新推导绑定：巡检组未变时保留原 instance_mode，
  组变了则用默认值。前端与 `validateMountBinding` 对齐做**双向联动适应**：
  改巡检对象→组下拉只保留兼容组（项目/环境对象仅通用组；逻辑服务对象仅同应用的应用组），
  当前组不兼容时自动切到第一个可用组并重推挂载；改巡检组→对象自动适应
  （应用组遇项目/环境对象时自动落到同项目优先同环境的第一个逻辑服务，范围内无服务才清空提示）。
