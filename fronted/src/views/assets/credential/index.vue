...
const resolveSelectedDeleteItems = () => {
    ...
    if (selectedRows.length) {
        return selectedRows.map((item) => `凭证: ${item.name || `#${item.id}`}`)
    }
    return selectedIds.map((id) => `凭证ID: ${id}`)
}
...
const openBatchDeleteConfirm = () => {
    ...
    openDeleteConfirm({
        title: '确认批量删除凭证',
        summary: `即将删除 ${selectedIds.length} 项凭证，请确认影响清单。`,
...
const openDeleteRecordConfirm = (record) => {
    openDeleteConfirm({
        title: '确认删除凭证',
        summary: '删除后不可恢复。',
        items: [`凭证: ${record?.name || `#${record?.id || '-'}`}`],
