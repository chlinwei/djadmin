            ...
            <a-space>
              ...
              <a-tooltip title="删除">
                <a-button
                  ...
                  type="primary"
                  danger
                  v-permission="'automation:playbooks:delete'"
                  @click="openDeletePlaybookConfirm(record)"
                >
...
function resolvePlaybookOrdering() {
  if (!playbookSort.field || !playbookSort.order) {
    return '-id'
  }
  const prefix = playbookSort.order === 'descend' ? '-' : ''
  return `${prefix}${playbookSort.field}`
}
...
async function loadPlaybooks() {
  ...
  try {
    const res = await getPlaybookList({
      ...
      ordering: resolvePlaybookOrdering(),
    })
...
function openDeletePlaybookConfirm(record) {
  const name = String(record?.name || '').trim() || `#${record?.id || '-'}`
  openDeleteConfirm({
    title: '确认删除模板',
    summary: '删除后不可恢复，请确认影响清单。',
    items: [`模板: ${name}`],
    onConfirm: () => onDeletePlaybook(record),
  })
}
...
function handlePlaybookTableChange(page, _filters, sorter) {
  ...
  if (nextSorter?.field && allowedFields.includes(nextSorter.field) && nextSorter.order) {
    playbookSort.field = nextSorter.field
    playbookSort.order = nextSorter.order
  } else {
    playbookSort.field = null
    playbookSort.order = null
  }
