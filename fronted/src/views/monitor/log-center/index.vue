<template>
  <div class="log-center">
    <!-- 日志中心：把「日志存储水位」「日志查询」「逻辑服务的日志配置」三块合成一个入口——
          ...
          <template v-else>
            ...
            <div class="apply-bar">
              <div class="apply-left">
                ...
                <a-tooltip title="关闭时该服务下所有实例均不采集；逐条日志的开关随之不生效（配置仍保留，重新打开即恢复）。改动需要重新下发采集配置才在主机上生效。" placement="top">
            ...
            >
              <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'collection_enabled'">
                  ...
                  <a-tooltip title="关闭表示本服务不再采集这条日志（同一模板下其他服务不受影响）；改动需要重新下发采集配置才在主机上生效。" placement="top">
                    <a-switch
                      :checked="isLogCollected(record)"
                      :loading="Boolean(savingRows[record.log_definition])"
                      checked-children="采"
                      un-checked-children="不采"
                      @change="(checked) => saveOverride(record, { collection_enabled: checked ? null : false })"
                    />
                  </a-tooltip>
                </template>
                ...
                <template v-else-if="column.key === 'retention_tier'">
                  ...
                  <a-tooltip title="改档位会写入新流（旧流停止写入、按原档位保留到期，不迁移数据）；改动需要重新下发采集配置后主机才会走新流。" placement="top">
                    <a-select
                      ...
                      @update:value="(value) => saveOverride(record, { retention_tier: value ?? null })"
                    />
                  </a-tooltip>
                </template>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'actions'">
                <template v-if="isHistoricalTier(record)">
                  ...
                  <a-tooltip title="删除这条流里的全部文档、立刻释放磁盘。不可恢复；留空流对象（切回该档位会继续写入它）。" placement="top">
                    <a-button type="link" size="small" danger :loading="Boolean(cleaningStream[record.name])" @click="cleanupHistoricalStream(record)">立即清理</a-button>
                  </a-tooltip>
                </template>
                <span v-else class="apply-muted">-</span>
              </template>
...
import { applyLogTargetsForService, cleanupLogDataStream, getElasticsearchClusterList, getLogBatchJob, getLogRetentionTiers, getLogStorageOverview, getServiceLogConfigState } from '@/api/monitor'
import ServiceTree from '@/views/assets/application/components/ServiceTree.vue'
import LogQueryPanel from '@/views/assets/service-tree/LogQueryPanel.vue'
...
// 服务级采集状态的判据与存储水位页一致：服务停用 / 未开采集时该流不会再写入新配置，
...
async function saveOverride(record, patch) {
  ...
  savingRows[key] = true
  try {
    const response = await saveApplicationServiceLogSetting(serviceId.value, {
      log_definition_id: key,
      collection_enabled: 'collection_enabled' in patch ? patch.collection_enabled : (record.collection_enabled ?? null),
      retention_tier: 'retention_tier' in patch ? patch.retention_tier : (record.retention_tier ?? null),
    })
...
async function submitSwitchTier() {
  ...
  for (const row of selected) {
    try {
      // 接口把提交体当作该行覆盖值的全集：采集开关必须带上当前值，否则会被清成"不覆盖"。
      await saveApplicationServiceLogSetting(serviceId.value, {
        log_definition_id: row.log_definition,
        retention_tier: tierId,
        collection_enabled: row.collection_enabled ?? null,
      })
      ok += 1
    } catch (error) {
...
// 按流清理：删这条流里的全部文档（异步任务），不可恢复；流对象留着，切回档位还能继续写。
async function cleanupHistoricalStream(stream) {
  const tier = stream?.tier
  if (!tier) return
  await openDeleteConfirm({
    title: '确认清理这条历史档位的数据流',
    summary: '会删除这条流里的全部文档以释放磁盘，删除不可恢复。流对象保留（切回该档位仍会写入它）；'
      + '只影响这个档位，本服务其他档位的数据不受影响。删除在 Elasticsearch 侧异步执行。',
    items: [`${stream.name}（档位 ${tier}，${formatBytes(stream.bytes)}）`],
    onConfirm: async () => {
      cleaningStream[stream.name] = true
      try {
        await cleanupLogDataStream({ service_id: serviceId.value, mode: 'all', tier })
        message.success('清理任务已提交（异步执行），稍后刷新可看到占用下降')
        await loadStorage()
      } finally {
        delete cleaningStream[stream.name]
      }
    },
  })
}
...
async function toggleServiceCollect(checked) {
  ...
  try {
    ...
    // 总开关会改变渲染内容（该服务的片段整体出现/消失）→ 主机配置态随之变化，所以要刷新下发状态。
    await loadApplyState()
  } catch (error) {
